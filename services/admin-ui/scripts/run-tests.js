...
try {
  ...
  // Run tests with Docker configuration (uses existing container) 
  // Only run in Chromium to avoid browser installation issues
  execSync('npx playwright test -c playwright.docker.config.ts --reporter=line --project=chromium', { 
    stdio: 'inherit' 
  });
