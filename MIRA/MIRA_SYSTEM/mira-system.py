...
A practical development assistance system that combines genuine memory integration
with lightweight code analysis tools.
...
Usage Examples:
  ...
  # Core Operations
  python mira-system.py --quick         # Fast health check (<5s)
  python mira-system.py --analyze       # Deep analysis with memory
  python mira-system.py --heal          # Auto-fix issues
  
  # Memory-Enhanced Intelligence
  python mira-system.py --memory-context    # Show memory insights
  python mira-system.py --learn             # Pattern learning from history
"""
...
# Add the MIRA_SYSTEM directory to the Python path
MIRA_SYSTEM_DIR = Path(__file__).parent
sys.path.insert(0, str(MIRA_SYSTEM_DIR))
...
from core.system import MIRAQualitySystem
...
class MIRASystem:
    """
    MIRA system focused on memory integration and practical code analysis
    """
    ...
    def __init__(self, project_root: Path):
        ...
        self.quality_system = MIRAQualitySystem(project_root)
...
Examples:
  ...
  python claude-system.py --team-analysis      # Multi-perspective analysis
    ...
    system = MIRASystem(project_root)
